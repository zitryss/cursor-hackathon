...
export function meta() {
  return [
    { title: "Year File — Is this deductible?" },
    {
      name: "description",
      content:
...
export default function YearFilePulseRoute() {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isFinite(amountEuro) || amountEuro <= 0) {
      setFormError("Enter an amount in € so we can sketch the YTD impact.");
      return;
    }
    ...
    >
      <header className="space-y-3 border-b border-frame-ink pb-4">
        ...
        <p className="font-body text-body text-muted-foreground">
          This week’s spend. A plain answer. Pocket the tax save now — don’t wait for July.
        </p>
      </header>
      ...
      >
        ...
        <p className="mt-2 font-body text-caption text-muted-foreground">
          Open next week to see how the score moved — no streaks, no nags, just
          a cleaner April you.
        </p>
      </section>
      ...
      >
        ...
        <p className="mt-1 font-body text-body-sm text-muted-foreground">
          Text first. Photo can wait — speed is the flex.
        </p>
        ...
        <form className="mt-4 space-y-3" onSubmit={onSubmit}>
          <label className="block space-y-1">
            ...
            <input
              ...
              name="description"
              placeholder="e.g. coworking day pass, Fachliteratur, Bahn to client"
              value={description}
      ...
      <section aria-label="Year File" className="space-y-3">
        <div className="flex items-baseline justify-between gap-2">
          <h2 className="font-ui text-heading-2">Year File</h2>
        ...
        {ready && yearFile.expenses.length === 0 ? (
          <p className="border border-dashed border-frame-ink bg-card p-4 font-body text-body-sm text-muted-foreground">
            Empty on purpose. The first pulse starts the dossier.
          </p>
        ) : (
      ...
      <footer className="mt-auto border-t border-frame-ink pt-4">
        ...
        <p className="mt-1 font-body text-body-sm text-muted-foreground">
          Sal → Hackermans scope → cursor on Sapne · Titans TEAM-PLAN · Shirley/Sam ship.
          Built with Cursor + agents — one weekly pulse, not a committee.
        </p>
      </footer>
    </main>
  );
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 border-t border-frame-ink/40 pt-2 font-body text-caption">
        Saved to your Year File. Come back next week — the weekly save total is
        the reason to return, not a notification.
      </p>
    </section>
  );
}
