...
export const DEMO_WEEK_SEEDS: readonly {
	description: string;
	amountEuro: number;
}[] = [
	{ description: "coworking day pass", amountEuro: 45 },
	{ description: "Bahn to client meeting", amountEuro: 28.5 },
	{ description: "Netflix", amountEuro: 12.99 },
];
...
/** One-click demo: work / maybe / no seeds for tonight's video path. */
export function loadDemoWeek(): YearFileState {
	let state = emptyYearFile();
	let last: YearFileExpense | null = null;
	for (const seed of DEMO_WEEK_SEEDS) {
		const result = addExpenseToYearFile(state, seed);
		state = result.state;
		last = result.expense;
	}
	void last;
